...
# ---- CALCULATE METRICS FOR DEVIATION REPORT ----
# These calculations must be done BEFORE the download sections
if df_shopify is not None and df_final_campaign is not None and 'df_final_campaign' in locals() and not df_final_campaign.empty:
    st.info("📊 Calculating metrics for deviation report...")
    
    # Get unique dates from campaign data
    if 'unique_dates' not in locals() or not unique_dates:
        unique_dates = sorted([str(d) for d in df_final_campaign['Date'].unique() 
                              if pd.notna(d) and str(d).strip() != ''])
    
    # Ensure day-wise lookups exist (they should already be created, but double-check)
    if 'product_date_avg_prices' not in locals() or not product_date_avg_prices:
    ...
    # Calculate product-level averages
    if 'product_total_avg_prices' not in locals() or not product_total_avg_prices:
        product_total_avg_prices = {}
        product_total_delivery_rates = {}
        
        for product, product_df in df_final_campaign.groupby("Product"):
    ...
    if 'product_net_profit_values' not in locals() or not product_net_profit_values:
        ...
        for product, product_df in df_final_campaign.groupby("Product"):
            ...
            for date in unique_dates:
                ...
                for _, campaign_row in date_data.iterrows():
                    ...
                    total_net_profit_for_product += campaign_date_net_profit
            
            product_net_profit_values[product] = round(total_net_profit_for_product, 2)
    
    st.success("✅ Metrics calculated successfully for deviation report")
# ==== END OF NEW SECTION ====
...
def convert_final_campaign_to_excel_with_date_columns_fixed(df, shopify_df=None, selected_days=None):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        # Add comparison columns before remark
        all_columns.append("Purchases (Campaign)")
        all_columns.append("Net Items Sold (Shopify)")
        all_columns.append("Deviation %")
        
        # Build Shopify lookup dictionary for Net Items Sold by Canonical Product
        shopify_net_items_lookup = {}
        if df_shopify is not None and not df_shopify.empty:
            for product in df_shopify['Canonical Product'].unique():
                product_data = df_shopify[df_shopify['Canonical Product'] == product]
                total_net_items = product_data['Net items sold'].sum()
                shopify_net_items_lookup[product] = int(total_net_items)
            
            st.info(f"📊 Built Shopify lookup for {len(shopify_net_items_lookup)} products")
        ...
        start_col = 9  # Column H (after base columns A, B, C, D, E, F + separator G)
        total_columns = len(all_columns)
        ...
        # Consider "recently completed" and "inactive" as the same (Inactive)
                ...
                # NEW: Leave comparison columns EMPTY for campaign rows (only show at product total level)
            ...
            if campaign_rows:
                ...
                # NEW: Write Purchases (Campaign), Net Items Sold (Shopify), Deviation % for product totals AT THE END
                purchases_campaign_col_idx = all_columns.index("Purchases (Campaign)")
                net_items_shopify_col_idx = all_columns.index("Net Items Sold (Shopify)")
                deviation_col_idx = all_columns.index("Deviation %")
                
                # Purchases (Campaign) - link to Total_Purchases
                total_purchases_col_idx = all_columns.index("Total_Purchases")
                worksheet.write_formula(
                    product_total_row_idx, purchases_campaign_col_idx,
                    f"={xl_col_to_name(total_purchases_col_idx)}{product_total_row_idx+1}",
                    product_total_format
                )
                
                # Net Items Sold (Shopify) - value from lookup
                shopify_net_items = shopify_net_items_lookup.get(product, 0)
                safe_write(worksheet, product_total_row_idx, net_items_shopify_col_idx, shopify_net_items, product_total_format)
                
                # Deviation % - ALWAYS write formula first, then apply conditional formatting
                purchases_col_letter = xl_col_to_name(purchases_campaign_col_idx)
                shopify_col_letter = xl_col_to_name(net_items_shopify_col_idx)
                
                # Write the deviation formula with default format
                worksheet.write_formula(
                    product_total_row_idx, deviation_col_idx,
                    f"=IF({shopify_col_letter}{product_total_row_idx+1}=0,0,ROUND(({shopify_col_letter}{product_total_row_idx+1}-{purchases_col_letter}{product_total_row_idx+1})/{shopify_col_letter}{product_total_row_idx+1}*100,2))",
                    product_total_format
                )
                
                # Apply conditional formatting: RED if deviation < -10
    ...
    # Round the sum for this date (matching Excel's date-column rounding)
        ...
        if product_total_rows:
            ...
            # NEW: Write Purchases (Campaign), Net Items Sold (Shopify), Deviation % for grand total AT THE END
            purchases_campaign_col_idx = all_columns.index("Purchases (Campaign)")
            net_items_shopify_col_idx = all_columns.index("Net Items Sold (Shopify)")
            deviation_col_idx = all_columns.index("Deviation %")
            ...
            # Net Items Sold (Shopify) - sum of all product totals
            if product_total_rows:
                sum_refs = []
                for product_row_idx in product_total_rows:
                    product_excel_row = product_row_idx + 1
                    sum_refs.append(f"{xl_col_to_name(net_items_shopify_col_idx)}{product_excel_row}")
                ...
                safe_write(worksheet, grand_total_row_idx, net_items_shopify_col_idx, 0, grand_total_format)
            
            # Deviation % - formula with RED background if < -10
            purchases_col_letter = xl_col_to_name(purchases_campaign_col_idx)
            shopify_col_letter = xl_col_to_name(net_items_shopify_col_idx)
            
            
            # Calculate grand total deviation correctly (Shopify - Campaign) / Shopify
            grand_purchases = df_main['Purchases'].sum() if 'Purchases' in df_main.columns else 0
            grand_shopify = sum(shopify_net_items_lookup.values())
            grand_deviation = round((grand_shopify - grand_purchases) / grand_shopify * 100, 2) if grand_shopify > 0 else 0
            
            # Red background when absolute deviation > 10 (both < -10 OR > 10)
            if grand_deviation < -10:
                grand_deviation_format = workbook.add_format({
                    "bold": True, "align": "left", "valign": "vcenter",
                    "fg_color": "#FF0000", "font_name": "Calibri", "font_size": 11,
                    "num_format": "#,##0.00"
                })
            else:
                grand_deviation_format = grand_total_format
            
            # FIXED: Deviation = (Net Items Sold - Purchases) / Net Items Sold * 100
            worksheet.write_formula(
                grand_total_row_idx, deviation_col_idx,
                f"=IF({shopify_col_letter}{grand_total_row_idx+1}=0,0,ROUND(({shopify_col_letter}{grand_total_row_idx+1}-{purchases_col_letter}{grand_total_row_idx+1})/{shopify_col_letter}{grand_total_row_idx+1}*100,2))",
                grand_deviation_format
            )
...
def create_deviation_report_from_data(df_shopify, df_campaign, product_net_profit_values, 
                                      product_total_avg_prices, product_total_delivery_rates,
                                      product_date_avg_prices, product_date_delivery_rates,
                                      product_date_cost_inputs, unique_dates,
                                      shipping_rate, operational_rate):
    """
    Create deviation report using actual calculated data from both Shopify and Campaign
    """
    from io import BytesIO
    import pandas as pd
    ...
    # ===== CALCULATE SHOPIFY NET PROFIT % FOR EACH PRODUCT =====
    shopify_products = {}
    ...
    if df_shopify is not None and not df_shopify.empty:
        for product, product_df in df_shopify.groupby("Canonical Product"):
            ...
            for _, row in product_df.iterrows():
                # Get values
                net_items = pd.to_numeric(row.get("Net items sold", 0), errors="coerce") or 0
                avg_price = pd.to_numeric(row.get("Product variant price", 0), errors="coerce") or 0
                delivery_rate_raw = row.get("Delivery Rate", 0)
                product_cost_input = pd.to_numeric(row.get("Product Cost (Input)", 0), errors="coerce") or 0
                ad_spend = pd.to_numeric(row.get("Ad Spend (USD)", 0), errors="coerce") or 0
                
                # Convert delivery rate
                if isinstance(delivery_rate_raw, str):
                    delivery_rate_raw = delivery_rate_raw.strip().replace("%", "")
                delivery_rate = pd.to_numeric(delivery_rate_raw, errors="coerce") or 0
                if delivery_rate > 1:
                    delivery_rate = delivery_rate / 100.0
                
                # Calculate metrics (EXACT same as Excel formulas)
                delivered_orders = round(net_items * delivery_rate, 2)
                net_revenue = round(delivered_orders * avg_price, 2)
                shipping_cost = round(net_items * shipping_rate, 2)
                operational_cost = round(net_items * operational_rate, 2)
                product_cost_output = round(delivered_orders * product_cost_input, 2)
                net_profit = round(net_revenue - (ad_spend * 100) - shipping_cost - operational_cost - product_cost_output, 2)
            ...
            if total_net_revenue > 0:
                shopify_products[product] = round((total_net_profit / total_net_revenue) * 100, 2)
            else:
                shopify_products[product] = 0.0
    
    st.info(f"📊 Calculated Net Profit % for {len(shopify_products)} products in Shopify")
    
    # ===== CALCULATE CAMPAIGN NET PROFIT % FOR EACH PRODUCT =====
    campaign_products = {}
    
    if df_campaign is not None and not df_campaign.empty:
        for product, product_df in df_campaign.groupby("Product"):
            # Get product-level values
            product_avg_price = round(product_total_avg_prices.get(product, 0), 2)
            product_delivery_rate = round(product_total_delivery_rates.get(product, 0), 2)
            
            # Calculate total net profit using DAY-BY-DAY approach (matching your existing code)
            total_net_profit_sum = 0
            total_purchases = 0
            
            # Get all dates for this campaign
            campaign_dates = sorted([str(d) for d in product_df['Date'].unique() if pd.notna(d) and str(d).strip() != ''])
                ...
                for date in campaign_dates:
                    ...
                    if not date_data.empty:
                        for _, row_data in date_data.iterrows():
                            ...
                            total_net_profit_sum += round(date_net_profit, 2)
                            total_purchases += date_purchases
                
                # Calculate Total Net Profit %
                calc_purchases_total = 1 if (total_purchases == 0 and product_df.get("Amount Spent (USD)", 0).sum() > 0) else total_purchases
                delivery_rate_total = round(product_delivery_rate / 100 if product_delivery_rate > 1 else product_delivery_rate, 2)
                
                numerator_total = round(total_net_profit_sum, 2)
                denominator_total = round(product_avg_price * calc_purchases_total * delivery_rate_total, 2)
                campaign_net_profit_pct = round((numerator_total / denominator_total * 100), 2) if denominator_total > 0 else 0
                
                campaign_products[product] = campaign_net_profit_pct
            else:
                campaign_products[product] = 0.0
    
    st.info(f"📊 Calculated Net Profit % for {len(campaign_products)} products in Campaign")
    
    # ===== COMPARE AND CREATE DEVIATION REPORT =====
    all_products = set(shopify_products.keys()) | set(campaign_products.keys())
    
    # Categorize products into priority groups
    critical_deviations = []  # Both values ≠ 0 AND deviated
    ok_products = []  # Both values ≠ 0 AND OK
    reference_only = []  # Only in one file
    
    for product in sorted(all_products):
        shopify_value = shopify_products.get(product, None)
        campaign_value = campaign_products.get(product, None)
        
        # Handle missing values
        if shopify_value is None and campaign_value is None:
            continue
        elif shopify_value is None:
            deviation = None
            status = "⚠️ Only in Campaign"
            reference_only.append({
                'Product Name': product,
                'Shopify Net Profit (%)': "N/A",
                'Campaign Net Profit (%)': round(campaign_value, 2) if campaign_value is not None else "N/A",
                'Deviation': "N/A",
                'Status': status
            })
        elif campaign_value is None:
            deviation = None
            status = "⚠️ Only in Shopify"
            reference_only.append({
                'Product Name': product,
                'Shopify Net Profit (%)': round(shopify_value, 2) if shopify_value is not None else "N/A",
                'Campaign Net Profit (%)': "N/A",
                'Deviation': "N/A",
                'Status': status
            })
        else:
            # Both values exist
            deviation = campaign_value - shopify_value
            
            # Determine status
            if abs(deviation) > 2.0:
                status = f"⚠️ DEVIATED by {deviation:+.2f}%"
                critical_deviations.append({
                    'Product Name': product,
                    'Shopify Net Profit (%)': round(shopify_value, 2),
                    'Campaign Net Profit (%)': round(campaign_value, 2),
                    'Deviation': round(deviation, 2),
                    'Status': status,
                    'abs_deviation': abs(deviation)  # For sorting
                })
            else:
                status = "✅ OK"
                ok_products.append({
    ...
    # Sort critical deviations by absolute deviation (highest first)
    critical_deviations.sort(key=lambda x: x['abs_deviation'], reverse=True)
    
    # Remove the temporary sorting key
    for item in critical_deviations:
        del item['abs_deviation']
    
    # Combine in priority order: Critical Deviations → OK → Reference Only
    comparison_data = critical_deviations + ok_products + reference_only
    
    if not comparison_data:
        st.warning("⚠️ No products found for comparison")
        return None
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        df_comparison.to_excel(writer, index=False, sheet_name="Deviation Report")
        ...
        # CRITICAL DEVIATION FORMAT (Red - highest priority)
        ...
        ok_format = workbook.add_format({
            "align": "left", "valign": "vcenter",
            "fg_color": "#E8F5E9", "font_name": "Calibri", "font_size": 11,
            "num_format": "#,##0.00"
        })
        
        # REFERENCE ONLY FORMAT (Yellow - low priority)
        warning_format = workbook.add_format({
            "align": "left", "valign": "vcenter",
            "fg_color": "#FFF9C4", "font_name": "Calibri", "font_size": 11
        })
        
        # Write headers
        for col_num, col_name in enumerate(df_comparison.columns):
            worksheet.write(0, col_num, col_name, header_format)
        
        # Write data with conditional formatting
        for row_num in range(len(df_comparison)):
            status = df_comparison.iloc[row_num]['Status']
            
            for col_num in range(len(df_comparison.columns)):
                cell_value = df_comparison.iloc[row_num, col_num]
                
                if '⚠️ DEVIATED' in status:
                    worksheet.write(row_num + 1, col_num, cell_value, critical_format)
                elif '⚠️ Only' in status:
                    worksheet.write(row_num + 1, col_num, cell_value, warning_format)
                else:
                    worksheet.write(row_num + 1, col_num, cell_value, ok_format)
        
        # Set column widths
        worksheet.set_column(0, 0, 35)  # Product Name
        worksheet.set_column(1, 1, 25)  # Shopify Net Profit (%)
        worksheet.set_column(2, 2, 25)  # Campaign Net Profit (%)
        worksheet.set_column(3, 3, 15)  # Deviation
        worksheet.set_column(4, 4, 30)  # Status
        
        # Add summary section
        summary_row = len(df_comparison) + 3
        ...
        worksheet.write(summary_row + 1, 0, f"Total Products Compared: {total_products}")
        worksheet.write(summary_row + 2, 0, f"🔴 CRITICAL: Products with BOTH values & DEVIATED (>2%): {critical_count}", critical_format)
        worksheet.write(summary_row + 3, 0, f"✅ OK: Products with BOTH values & OK (≤2%): {ok_count}", ok_format)
        worksheet.write(summary_row + 4, 0, f"📋 REFERENCE: Products only in one file: {reference_count}", warning_format)
        
        # Add separator lines for visual clarity
        separator_row_1 = 1 + len(critical_deviations)  # After critical deviations
        separator_row_2 = separator_row_1 + len(ok_products)  # After OK products
        
        if critical_count > 0 and (ok_count > 0 or reference_count > 0):
            # Add a visual separator after critical deviations
            separator_format = workbook.add_format({
                "bg_color": "#4472C4",
                "bold": True
            })
            for col in range(5):
                worksheet.write(separator_row_1, col, "", separator_format)
        
        if ok_count > 0 and reference_count > 0:
            # Add a visual separator after OK products
            separator_format_2 = workbook.add_format({
                "bg_color": "#90CAF9"
            })
            for col in range(5):
                worksheet.write(separator_row_2, col, "", separator_format_2)
    
    st.success(f"✅ Deviation report created with {len(comparison_data)} products")
    st.info(f"🔴 {critical_count} Critical (Deviated) | ✅ {ok_count} OK | 📋 {reference_count} Reference Only")
...
# After the Campaign download button, add:

# ---- DEVIATION REPORT DOWNLOAD ----
if campaign_files and shopify_files:
    st.header("📊 Deviation Report")
    
    # Check if we have all the required data
    if (df_shopify is not None and not df_shopify.empty and 
        df_final_campaign is not None and not df_final_campaign.empty and
        'product_net_profit_values' in locals() and
        'product_total_avg_prices' in locals() and
        'product_total_delivery_rates' in locals() and
        'product_date_avg_prices' in locals() and
        'product_date_delivery_rates' in locals() and
        'product_date_cost_inputs' in locals() and
        'unique_dates' in locals()):
        
        try:
            # Create the deviation report
            deviation_report = create_deviation_report_from_data(
                df_shopify=df_shopify,
                df_campaign=df_final_campaign,
                product_net_profit_values=product_net_profit_values,
                product_total_avg_prices=product_total_avg_prices,
                product_total_delivery_rates=product_total_delivery_rates,
            ...
            if deviation_report:
                st.download_button(
                    label="📊 Download Shopify vs Campaign Deviation Report",
                    data=deviation_report,
                    file_name="deviation_report.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )
        except Exception as e:
            st.error(f"❌ Error creating deviation report: {str(e)}")
            import traceback
            st.code(traceback.format_exc())
    else:
        st.warning("⚠️ Please process both Shopify and Campaign files to generate deviation report")
